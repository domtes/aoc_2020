...
for freq, symbol, password in data:
    freq_min, freq_max = (int(n) for n in freq.split("-"))
    symbol = symbol[:1]

    frequencies = Counter(password)
    f = frequencies[symbol]

    if f >= freq_min and f <= freq_max:
        old_valid_count += 1

    if (password[freq_min - 1] == symbol) != (password[freq_max - 1] == symbol):
        new_valid_count += 1
